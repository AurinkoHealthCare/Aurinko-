  ...
  const fetchHeaderImage = async () => {
    try {
      ...
      const selected = arr.find(
        (img) => img.imageName?.toLowerCase() === "livestock"
      );
    ...
    <div className={`font-sans ${theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-black"}`}>
      ...
      <div className="relative w-full">
        {loadingHeader ? (
          ...
          <div className="w-full h-64 flex items-center justify-center bg-gray-100">
            <p className="text-gray-500">No "Livestock" image found</p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col w-full p-4">
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-4">
          <ProductCard products={products} segment="Livestock" theme="Veterinary" />
        </div>
      </div>
    </div>
  );
