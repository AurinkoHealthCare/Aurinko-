  ...
  return (
    <div className={`font-sans ${theme === "dark" ? "bg-gray-900 text-white" : "bg-white text-black"}`}>
      <div className="relative">
        <img
          src="/Assets/Nano-biotechnology Compounds/Neunamin.webp"
          alt="Health supplements"
          className="w-full"
        />
        <div className="absolute inset-0 bg-black opacity-50"></div>
        <div className="absolute inset-0 flex flex-col items-center text-white p-8">
          <h1 className="lg:text-4xl font-bold">Neunamin</h1>
        </div>
      </div>
      <div className="flex flex-col min-h-screen w-full p-4">
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-4">
          <ProductCard products={products} segment="Neunamin" theme="Human" />
        </div>
      </div>
    </div>
  );
